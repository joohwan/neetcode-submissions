class Solution {
    public int networkDelayTime(int[][] times, int n, int k) {
        ...
        Set<Integer> checkedNeighbors = new HashSet<>(); // avoid cycle
        ...
        while (!q.isEmpty()) {
            ...
            for (long[] toNodeAndTime : connectedEdges) {
                ...
                if (!checkedNeighbors.contains(toNode) && timeTillFrom + time < travelTimes[toNode]) {
                    travelTimes[toNode] = timeTillFrom + time;
                    q.offer(new long[] {toNode, travelTimes[toNode]});
                }
            }
            checkedNeighbors.add(fromNode);
        }
